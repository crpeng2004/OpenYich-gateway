...
/**
 * Decides when a JWT string is valid. First try to parse it, then check that the signature is
 * correct. If something fails an empty Mono is returning meaning that is not valid. Verify that
...
public class JWTCustomVerifier {
  ...
  public Mono<SignedJWT> check(String token) {
    return Mono.justOrEmpty(createJWS(token)).filter(isNotExpired).filter(validSignature);
  }

  private Predicate<SignedJWT> isNotExpired =
      token -> getExpirationDate(token).after(Date.from(Instant.now()));
